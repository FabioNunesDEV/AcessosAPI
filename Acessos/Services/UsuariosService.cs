...
public class UsuariosService
{
    ...
    public UsuarioReadDTO ObterUsuarioPorId(int id)
    {
        ...
        var usuario = this.ObterUsuarioCadastradoPorId(id);

        return _mapper.Map<UsuarioReadDTO>(usuario);
    }

    public UsuarioReadDTO ObterUsuarioPorLogin(string login)
    {
        this.Validarlogin(login);

        var usuario = this.ObterUsuarioCadastradoPorLogin(login);
    ...
    {
        ValidarId(id);
        var usuario = ObterUsuarioCadastradoPorId(id);
    ...
    public void AdicionarUsuarioGrupo(int usuarioId, int grupoId)
    {
        ...
        var usuario = ObterUsuarioCadastradoPorId(usuarioId);
    ...
    private void Validarlogin(string login)
    {
        ...
        {
            throw new ArgumentException("O login não pode estar em branco");
        }
    }

    private Usuario ObterUsuarioCadastradoPorId(int id)
    {
    ...
    private Usuario ObterUsuarioCadastradoPorLogin(string login)
    {
        var usuario = _context.Usuarios.FirstOrDefault(u => u.Login == login);
        if (usuario == null)
        {
            throw new ArgumentException($"Usuário com login {login} não encontrado.");
        }
        return usuario;
    }

    private void LoginJaExiste(string login)
    {
        bool existe = _context.Usuarios.Any(u => u.Login == login);

        if (existe == true)
        {
            throw new ArgumentException($"Login {login} já existe.");
        }  
    }
}
